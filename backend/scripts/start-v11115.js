...
const runtimePath = path.resolve(__dirname, './start-v11115.runtime.js');

let source = await fs.readFile(sourcePath, 'utf8');

// V111.15 — mantém a mídia original da V111.14 e corrige MIME/preview de imagens.
...
const newMediaBody = `  const originalName = String(req.file.originalname || 'midia').slice(0, 240);
  ...
  const imageExts = new Set(imageMimeByExt.keys());
  ...
  const inferredImageMime = imageMimeByExt.get(ext) || '';
  let storedMime = isImage
    ? (mimeType.startsWith('image/') ? mimeType : inferredImageMime || 'application/octet-stream')
    : (mimeType || 'video/mp4');
  if (storedMime === 'image/jpg' || storedMime === 'image/pjpeg') storedMime = 'image/jpeg';
...
if (!source.includes(oldMediaBody)) throw new Error('V111.15: corpo da rota de mídia não encontrado.');
source = source.replace(oldMediaBody, newMediaBody);

const imageRouteMarker = `app.get('/media/leme-art/:mediaId', async (req, res) => {`;
if (!source.includes(imageRouteMarker)) throw new Error('V111.15: rota de leitura de mídia não encontrada.');
source = source.replace(imageRouteMarker, `app.get('/media/leme-art-image/:mediaId', async (req, res) => {
  ...
  const fileName = String(found.rows[0].file_name || 'imagem');
  const storedMime = String(found.rows[0].mime_type || '').toLowerCase();
  const ext = path.extname(fileName).toLowerCase();
  const imageMimeByExt = new Map([
    ['.png','image/png'],['.jpg','image/jpeg'],['.jpeg','image/jpeg'],['.webp','image/webp'],
    ['.avif','image/avif'],['.gif','image/gif'],['.bmp','image/bmp'],['.svg','image/svg+xml']
  ]);
  let responseMime = storedMime.startsWith('image/') ? storedMime : (imageMimeByExt.get(ext) || 'application/octet-stream');
  if (responseMime === 'image/jpg' || responseMime === 'image/pjpeg') responseMime = 'image/jpeg';
  const body = found.rows[0].file_data;

  res.setHeader('Content-Type', responseMime);
  res.setHeader('Content-Length', String(body?.length || 0));
...
if (!source.includes(mediaNeedle)) throw new Error('V111.15: rota de mídia não encontrada após normalização.');
...
const normalizedReplacement = `        cropX: Math.max(0, Math.min(100, Number(item.cropX ?? 50))) / 100,
        ...
        start, end, length: end - start,`;
if (!source.includes(normalizedNeedle)) throw new Error('V111.15: bloco de enquadramento do FFmpeg não encontrado.');
...
if (!source.includes(filterNeedle)) throw new Error('V111.15: filtro de vídeo do FFmpeg não encontrado.');
